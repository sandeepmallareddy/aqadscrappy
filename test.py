...
def getAQAD(phpsess,tauID,grade,date):
...
#Open CSV file and write
  ...
  with open('aqad.csv', 'ab') as csvfile:
        rowwriter = csv.writer(csvfile)
        rowwriter.writerow([date,grade,sub,ques,opA,opB,opC,opD,corr_ans])

  print "Gotcha Question {0}".format(jsonobj['qcode']);
 else:
  print "No record"
 
getAQAD(phpsess,tauID,6,'2016-06-26')
